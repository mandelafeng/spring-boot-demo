...
class LocalDateTimeTest {
  ...
  void test8() {
    ...
    // 时间的区间
    Duration between = Duration.between(LocalDateTime.now(), LocalDateTime.now().plusDays(5));
    between.getUnits().forEach(System.out::println);
    System.out.println(between.get(ChronoUnit.SECONDS));
    // 日期的时间
    Period between1 = Period.between(LocalDate.now(), LocalDate.now().plusDays(5));
